...
def encrypt_message(message):
    # 16 bytes that are crypto secure
    iv = os.urandom(AES.block_size) 
    # Convert passphrase into bytes using utf-8 encoding
    secret_key = bytes(SECRET_KEY_PASSPHRASE, encoding="utf8")

    # Pad message to create equal message blocks of 16, then convert to bytes
    plain_text = bytes(pad_message(message), encoding='utf8')

    # Use above variables to perform AES-CBC encryption using 128-bit key
    cipher = AES.new(secret_key, AES.MODE_CBC, iv)
    try:
        
        # Join IV to the cipher-text, then encode resulting string - reversable by the eval-server
...
# Computation to estimate the clock offset between 2 devices - in this case: local - ulta96, remote - laptop
